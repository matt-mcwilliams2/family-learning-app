...
function getDecoyLetters(word: string, count: number): string[] {
  ...
  const available = COMMON_LETTERS.filter((l) => !wordLetters.has(l));
  // Shuffle available
...
export function LetterTray({ word, onComplete }: LetterTrayProps) {
  const [built, setBuilt] = useState<BuiltLetter[]>([]);
  const [phase, setPhase] = useState<"building" | "answered">("building");
  const [correct, setCorrect] = useState(false);

  // Generate tray: word letters + 2-3 decoys, shuffled
  const tray: TrayTile[] = useMemo(() => {
    const wordLetters = word.word.toLowerCase().split("");
    const decoyCount = Math.min(3, Math.max(2, Math.ceil(wordLetters.length / 4)));
    const decoys = getDecoyLetters(word.word, decoyCount);
    const all = [...wordLetters, ...decoys].map((letter, i) => ({
      letter,
      id: i,
    }));
    return shuffle(all);
